import socket
import streamlit as st
from crc_input import append_crc, verify_packet

# Page configuration for a plain look
st.set_page_config(page_title="Data Monitor", layout="centered")

# --- Constants ---
HOST = "127.0.0.1"
PORT = 12345

# --- Auto-Connect Logic ---
if "sock" not in st.session_state:
    try:
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.settimeout(2)
        s.connect((HOST, PORT))
        st.session_state.sock = s
        st.session_state.connected = True
    except:
        st.session_state.sock = None
        st.session_state.connected = False

if "sent_log" not in st.session_state:
    st.session_state.sent_log = ""
if "recv_log" not in st.session_state:
    st.session_state.recv_log = ""
if "data_vals" not in st.session_state:
    st.session_state.data_vals = {}

# --- UI Layout ---
st.write("### Data Input (Decimal)")

# 6 Decimal Input Fields in a row (as per your drawing)
cols_input = st.columns(6)
inputs = []
for i in range(6):
    val = cols_input[i].number_input(f"In {i+1}", value=0, step=1, key=f"in_{i}")
    inputs.append(val)

if st.button("Send Request"):
    if st.session_state.connected:
        try:
            # Preparing raw bytes from 6 inputs
            raw_req = bytes(inputs)
            packet = append_crc(raw_req)
            st.session_state.sock.send(packet)
            st.session_state.sent_log = packet.hex().upper()
            
            # Receiving response
            resp = st.session_state.sock.recv(1024)
            st.session_state.recv_log = resp.hex().upper()
            
            if len(resp) >= 18:
                _, _, valid = verify_packet(resp)
                if valid:
                    st.session_state.data_vals = {
                        "HC": (resp[6] << 8) | resp[7],
                        "AS": (resp[8] << 8) | resp[9],
                        "CM": (resp[10] << 8) | resp[11],
                        "CP": (resp[12] << 8) | resp[13],
                        "CS": (resp[14] << 8) | resp[15],
                        "A1": resp[16],
                        "A2": resp[17]
                    }
        except Exception as e:
            st.error(f"Error: {e}")
    else:
        st.error("Server not connected.")

st.write("---")

# Data Display Area
col_left, col_right = st.columns(2)

with col_left:
    st.write("**Alarms**")
    if "A1" in st.session_state.data_vals:
        a1_byte = st.session_state.data_vals["A1"]
        for i in range(8):
            status = "ON" if (a1_byte >> i) & 1 else "OFF"
            st.text(f"Alarm {i+1}: {status}")
    else:
        for i in range(8): st.text(f"Alarm {i+1}: --")

with col_right:
    st.write("**Concentration**")
    labels = ["HS_Con", "AS_Con", "CM_Con", "CP_Con", "CS_Con"]
    keys = ["HC", "AS", "CM", "CP", "CS"]
    for label, key in zip(labels, keys):
        val = st.session_state.data_vals.get(key, "--")
        st.text(f"{label}: {val}")

st.write("---")

# CLI Style Logs at the bottom
st.text(f"Sent: {st.session_state.sent_log}")
st.text(f"Data Received: {st.session_state.recv_log}")

if not st.session_state.connected:
    st.warning("Status: Disconnected. Check if server is running.")